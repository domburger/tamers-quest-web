...
Respond with a JSON object {"name":"...","description":"...","rarity":int,"size":int,"tint":{"r":int,"g":int,"b":int}} — make the tint a colour that distinctly reads as this region on a minimap.`,
  ...
  tileDesignerSystem: `You are the DESIGNER agent for FLOOR TILES. Given a ground-type inspiration and its biome, you produce a tile: a short evocative NAME (1-3 words), a one-sentence description, a representative COLOUR as {r,g,b} (0-255, the base colour of this ground — the renderer adds grain/detail), a rarity 1-100, a slipperiness 0-10, an emissiveness 0-5 (how much the ground glows in the dark cave), and collidable 0 or 1 (1 = impassable, like deep water or lava — use sparingly). Pick a colour that fits BOTH the ground type and its biome. OPTIONALLY, if the ground is not perfectly uniform, add subtle per-edge colours "top","bottom","left","right" (each {r,g,b}, 0-255) that are SMALL variations of the base (a few shades lighter/darker — e.g. a top-lit surface is slightly lighter at the top, darker at the bottom). Keep them close to the base so same-type tiles still read as one coherent floor; they give the world generator richer tiling + rotation. Omit the edge fields for a flat, uniform surface. Respond ONLY with a JSON object: {"name":"...","description":"...","color":{"r":int,"g":int,"b":int},"rarity":int,"slipperiness":int,"emissiveness":int,"collidable":0,"top":{"r":int,"g":int,"b":int},"bottom":{"r":int,"g":int,"b":int},"left":{"r":int,"g":int,"b":int},"right":{"r":int,"g":int,"b":int},"visual":{"layers":[...]}} — the four edge fields are OPTIONAL; author "visual" per the VISUAL section below.`,
  tileDesignerUser: `Ground-type inspiration (2-4 words): {inspiration}
Biome: {biome}
Respond with a JSON object {"name":"...","description":"...","color":{"r":int,"g":int,"b":int},"rarity":int,"slipperiness":int,"emissiveness":int,"collidable":0,"visual":{"layers":[...]}} — the colour should fit this ground type within its biome, and author "visual" per the VISUAL section. You MAY also add optional subtle per-edge colours "top"/"bottom"/"left"/"right" (each {r,g,b}, small variations of the base) for richer tiling.`,
};
