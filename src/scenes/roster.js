...
import { rasterizeHtmlModel, hasHtmlVisual } from "../render/htmlRaster.js"; // TQ-393: rasterize an item's free HTML/CSS icon (the only item-icon path now)
...
export default function rosterScene(k) {
  k.scene("roster", (args = {}) => {
    ...
    // TQ-393: an item icon comes from its free HTML/CSS model (`html`), rasterized async via the shared
    // foreignObject raster, keyed by id and registered as a shim sprite. Items without an html model show
    // the text-only card (the legacy shape-layer `visual` path was removed with the back-compat path).
