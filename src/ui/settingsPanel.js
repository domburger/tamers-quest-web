...
export function drawSettingsPanel(k, rect, state) {
  ...
  const mp = k.mousePos();
  const { items, bottom } = layout(rect);
  state._maxScroll = Math.max(0, bottom - (ry + rh)); // 4 controls fit on every viewport; kept for contract symmetry
