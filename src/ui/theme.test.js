...
test("drawButton: paints fill+shadow+sheen+label; hover adds a glow layer; disabled drops it", () => {
  const base = mockDrawK();
  drawButton(base.k, { rect: [0, 0, 120, 40], text: "Buy" });
  // shadow + fill + sheen = 3 rects, 1 label, no glow when not hovered
  assert.equal(base.calls.rect.length, 3, "shadow + fill + sheen (no glow)");
  ...
  assert.equal(hov.calls.rect.length, 4, "hover adds a glow halo layer");

  const dis = mockDrawK();
  drawButton(dis.k, { rect: [0, 0, 120, 40], text: "Buy", hover: true, disabled: true });
  assert.equal(dis.calls.rect.length, 3, "disabled ignores hover → no glow layer");
});
