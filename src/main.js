...
// TQ-227 engine removal — CUTOVER FLIPPED (TQ-296, Dominik GO 2026-06-15): the hand-rolled raw-canvas2D
// backend (compat/canvasShim.js) is now the DEFAULT renderer. `k` is the canvas shim; the same init() +
// scene registration below runs on it (it exposes the full k.* surface). The Phaser path is the
// KILL-SWITCH fallback: `?backend=phaser` (or localStorage tq_backend=phaser) boots Phaser instead.
// Phaser is intentionally KEPT for the soak; its removal (the bundle drop) follows once canvas runs clean
// on real traffic.
