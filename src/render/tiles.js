...
// TQ-466: an impassable IN-GRID cell (collidable tile — a former-void boundary, or in-map water) is now
// just a flat DARK SHADE of its own colour, with NO wall bands. Drawn in PASS 2 at the exact cell so the
// dark/blocked region lines up with the cell-based collision hitbox (isWalkable: in-grid && !collidable)
// and any overlapping floor rim from a neighbour is covered. The tint keeps a little of the tile's colour
// so different biomes' blocked terrain still reads distinctly (a dark version of that ground).
const COLLIDABLE_DARK = 0.4; // multiply the tile colour → a clearly-darker "blocked" shade
function drawCollidableShade(k, x, y, E, t) {
  ...
  k.drawRect({ pos: k.vec2(x * E, y * E), width: E, height: E, color: k.rgb(r, g, b) });
}
...
export function drawTiles(k, map, camX, camY, cache, E, isExplored = null) {
  ...
  for (let x = x0; x <= x1; x++) {
    ...
    for (let y = y0; y <= y1; y++) {
      ...
      if (t.collidable) {
        drawCollidableShade(k, x, y, E, t); // TQ-466: impassable cell → flat dark shade at the exact cell (matches the hitbox)
        continue;
      }
