...
- ◑ **CN-12b Character-skin cosmetics in MP** — the new **character skins** (accent + cloak, `characterCosmetics.js`, Cosmetics store "Player Character" tab, `abe151a`) applied in **SP** (`game.js`) but not MP. ✅ **Self DONE 2026-06-07 (`@visual`):** `onlineGame` now draws **your** character with its equipped accent + cloak (mirrors SP) — safe for self since the camera centers you, so there's no self/rival colour-coding to preserve; default "azure" accent == the old hard-coded blue, so default players are unchanged. lint + 218 tests + build green. 📌 **DESIGN DECISION for rivals (flagged):** MP draws rivals with a hard-coded **red** accent so you can read who's a threat at a glance. Showing each rival's *cosmetic accent* would (a) need CN-12-style sync (server stores `charSkinId` → snapshot → render — straightforward) **and** (b) trade that self/rival colour-coding for cosmetic flair. **@user: for rivals in MP — (a) keep the red threat-coding (cosmetic accent self-only), (b) show cosmetic accents (lose red-coding), or (c) show the cosmetic *cloak* only + keep red accent?** I'll wire the sync + chosen render once you pick. `onlineGame.js`, (+ `net.js`/`world.js` for the rival-sync half).
