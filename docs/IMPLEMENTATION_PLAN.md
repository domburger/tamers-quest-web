...
- ✅ **VS-9 SP combat buttons now use `addButton`** — **DONE 2026-06-07 (`@visual`):** `fight.js makeBtn` now delegates to the themed `addButton` (glow/SFX/sheen/shadow/outline + MB-12 haptic), matching the rest of the game (onlineLobby was already migrated, `68d00c3`). Extended `addButton` with two backward-compatible opts: `tag` (applied to every layer so `clearButtons()`/`destroyAll(btnTag)` wipes the whole button — shadow/sheen/glow/label — between menu states) and `disabled` (greys unaffordable attacks → `surfaceAlt` fill + `textMut` ink, drops interaction). Both default to the prior behaviour, so every existing caller is unaffected. Verified: build clean + the SP lobby (also `addButton`, default path) renders unchanged via shoot-sp → the extension is non-regressive, and the combat buttons use the same primitive. *(A live combat-frame capture is RNG-gated — SP encounters need the player to roam onto a monster tile, which the blind harness didn't hit in 2 runs; a deterministic SP-combat harness is a QA follow-up.)* `fight.js`, `theme.js`.
