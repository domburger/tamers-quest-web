...
| Inventory view | `@unassigned` | **user-requested 2026-06-06** — in-run/lobby inventory UI: owned monsters + spirit chains (per-chain throws/durability, equipped), reusing `roster`/`shop` theme patterns |
| Settings/pause on Escape | `@unassigned` | **user-requested 2026-06-06** — ESC opens a pause/settings overlay (resume · audio mute · leave) in `onlineGame`/`game` instead of immediately quitting the round; reuse the settings scene |
| Red dots → character/monster models | `@unassigned` | **user-requested 2026-06-06** — replace rival **minimap red dots** (`onlineGame.js:157`) + remaining dot indicators with small character/monster glyphs (main-view rivals already use `drawCharacter`) |
| **Live asset-generation pipeline + admin controls** | `@unassigned` | **user-requested 2026-06-06** (extends P5 + P7-T5) — wire monster/asset generation live AND make it fully steerable in `/admin`: **model selection** (verify current OpenAI model IDs against OpenAI docs), **temperature + all model params**, and **editable prompts** (P7-T5 added prompt editing — extend with params/model picker); persist to DB `settings`. Big task |
