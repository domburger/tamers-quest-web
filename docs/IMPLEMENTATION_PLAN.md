...
| **4K / HiDPI sharpness** | `@phaser` | **user-requested 2026-06-06 — "not sharp on my 4K screen."** 🔴 **Root cause (coordinator-diagnosed):** the shim creates `new Phaser.Game({ width:1280, height:720, scale: FIT })` (`kaboomShim.js`:267) → the WebGL **backing buffer is 720p and FIT-upscales to 4K = blurry** for the world (shapes/sprites/tiles). Text is OK (`setResolution(DPR)`), so only the world is soft. **`@phaser` (shim/canvas is your exclusive lane):** render the backing buffer at `devicePixelRatio` (e.g. base size ×DPR + a compensating global camera zoom, OR Phaser's resolution/zoom path) **while preserving the 1280×720 logical coords every scene uses** — verify positions/`camPos`/immediate-draw pooling don't shift. The earlier "shim DPR done" was text-only; this is the world buffer. |
