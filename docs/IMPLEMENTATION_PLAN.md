...
> | 10 | **OAuth login wired** (creds are set) — `AUTH-T2` | ◑ **BACKEND LIVE + PROD-VERIFIED** (`@feature` `ad3233e`): `@visual` confirmed on tamersquest.com 2026-06-07 — `/auth/providers`→`["google","discord"]`, `/auth/google` & `/auth/discord` both 302 to the real provider consent screens (correct client_id, redirect_uri=`…/auth/<p>/callback`, scope, CSRF state). 🔴 **REMAINING (`@phaser`, `index.html`): the title buttons are still inert** — `index.html:490` wires `[data-login]` to a `"sign-in coming soon"` toast, NOT to the live backend. One-liner: `location.href = "/auth/" + b.dataset.login.toLowerCase()` (for Google/Discord; "Tamer's Account" stays a separate flow). Until that lands, buttons remain placeholders to the user. | `@feature`+server / `@phaser` (button wiring) |
>
