...
> | 10 | **OAuth login wired** (creds are set) — `AUTH-T2` | ◑ **BACKEND LIVE + PROD-VERIFIED** (`@feature` `ad3233e`): `@visual` confirmed on tamersquest.com 2026-06-07 — `/auth/providers`→`["google","discord"]`, `/auth/google` & `/auth/discord` both 302 to the real provider consent screens (correct client_id, redirect_uri=`…/auth/<p>/callback`, scope, CSRF state). **AUTH-T3 native email/password ALSO backend-complete + PROD-VERIFIED** (`a158e68`): `@visual` confirmed `POST /auth/signup` + `/auth/login` live on prod (GET→405, bad email→`{"error":"invalid_email"}`; hardened: timing-safe verify, throttle, no enumeration). 🔴 **REMAINING — THE highest-leverage one-liner in the repo (`@phaser`, `index.html`): ALL THREE login backends are live+ready but every title button is inert** — `index.html:490` wires `[data-login]` to a `"sign-in coming soon"` toast, NOT to the live backends. Google/Discord = `location.href = "/auth/" + b.dataset.login.toLowerCase()`; "Tamer's Account" = a small email/password form POSTing to `/auth/signup`|`/auth/login` then storing the returned `token`. **This is the clearest "built but not visible" gap on the board — 2 full backend tasks shipped, 0 reachable from the UI.** Until it lands, login looks unbuilt to the user. | `@feature`+server / `@phaser` (button wiring) |
>
