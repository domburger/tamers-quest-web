...
- 🔴 **CB-2 `damage:0` heal attacks** deal 1 to the enemy instead of healing the user (no heal action path). **⚠️ DESIGN-BLOCKED — needs `@feature`/user (`@visual` analysis 2026-06-07):** ~10 attacks have `damage:0` + healing descriptions; some carry `inflictedStatus:"Heal"/"Healing"` (currently inert per combat.js), and at least one ("divine attack…") *damages AND* heals (lifesteal). The data has **no heal-amount field**, so the fix needs decisions: (a) which moves heal — `damage<=0`, `inflictedStatus∈{Heal,Healing}`, or both? (b) heal amount — % max HP / power-based / fixed? (c) lifesteal vs pure-heal for the damage+heal ones? Then add the branch in `performAttack`. `engine/combat.js`, `attacks.json`. **🧭 `@coordinator` routing:** partly the **FGT-T1 a/b call** — under (b) AI-required combat the judge resolves heals (no rich engine branch); under (a) the *deterministic fallback* just needs a **crude** heal (e.g. `damage<=0` → heal ~25% max HP, tunable). So `@feature` can ship the crude deterministic branch **now** to kill the "deals 1 to enemy" bug; nuanced per-move amounts/lifesteal wait on the a/b decision. Not a hard user-blocker for the bugfix.
...
- 🟠 **CB-5 Energy stalemate** (no in-fight regen/Struggle) — two exhausted monsters skip forever (see Fix-first #4). **⚠️ Needs a design call (`@visual` note 2026-06-07):** the out-of-energy *skip* is intentional + **tested** (`combat.test.js` "insufficient energy skips the attack"), and a Q8 between-encounter energy "breather" already exists (`server/combat.test.js`) — so this is the *in-fight* edge case only. Fix = a free weak **Struggle** fallback, which changes the tested skip behavior + needs a Struggle damage value → `@feature`/user call. `engine/combat.js`. **🧭 `@coordinator`: NOT user-gated — `@feature` should just proceed** with a sensible default (Struggle = small fixed self-recoil-free damage, e.g. ~5% of attacker STR or a flat value, mark tunable) and update the existing skip test. Waiting on the user for a minor balance constant would stall a real stalemate bug; ship a default, the user tunes later.
