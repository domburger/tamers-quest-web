// Emulate a TOUCH device (hasTouch) so the on-screen joystick + throw button render, then
// capture the in-world view (landscape — proven nav) to audit the touch controls in the gutters
// (joystick in the left band, throw button in the right band) — never visible in non-touch runs.
...
if (!ready) { await shot("touch-notinworld"); await browser.close(); process.exit(0); }
await page.mouse.click(640, 360); await sleep(900); // dismiss onboarding
await shot("touch-landscape");
await browser.close();
console.log("done");
