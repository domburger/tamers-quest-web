// Lobby → Bestiary station → tap a grid cell to open the monster DETAIL popup, screenshot it.
...
// Bestiary station: left column index 4 → y ≈ 225 + 4*58 = 457.
await page.mouse.click(230, 457); await sleep(2500);
await shot("bd-00-bestiary");
// Tap the first grid cell (top-left of the grid) to open its detail popup.
await page.mouse.click(150, 200); await sleep(1200);
await shot("bd-01-detail");
// Tap again to close (tap anywhere closes detail).
await page.mouse.click(640, 360); await sleep(800);
await shot("bd-02-closed");
await browser.close();
console.log("done");
